...
const LABELS = { cod:"COD", nome:"Nome", r:"Ruolo", sq:"Squadra", p:"Presenze", mvt:"Mediavoto (Tot)", mvc:"Mediavoto (Casa)", mvf:"Mediavoto (Fuori)", fmt:"Fantamedia (Tot)", fmc:"Fantamedia (Casa)", fmf:"Fantamedia (Fuori)", gf:"Gol", gfr:"Gol su Rigore", gs:"Gol Subiti", gsr:"GS su Rigore", as:"Assist", a:"Ammonizioni", e:"Espulsioni", rp:"Rigori Parati", rs:"Rigori Sbagliati", ag:"Autogol", aff:"Affidabilità %", cambio_squadra:"Cambio Squadra", cambio_ruolo:"Cambio Ruolo" };

const LABELS_STORICO = { p_tot_4stag:"Presenze tot", cambio_ruolo_descr:"Cambio Ruolo", p_media_stagione:"Pres. medie/stagione", aff_media_stagione:"Aff. media/stagione", mvt_media_stagione:"MV media/stagione", fmt_media_stagione:"FM media/stagione", gf_media_stagione:"Gol medi/stagione", as_media_stagione:"Assist medi/stagione", gs_media_stagione:"GS medi/stagione", rp_media_stagione:"RP medi/stagione", a_media_stagione:"Amm. medie/stagione", e_media_stagione:"Esp. medie/stagione", presenze_per_gol:"Presenze per gol", presenze_per_assist:"Presenze per assist", presenze_per_ammonizione:"Presenze per amm.", presenze_per_espulsione:"Presenze per esp.", gs_per_presenza:"GS per presenza", rp_per_presenza:"RP per presenza", presenze_per_rp:"Presenze per RP", gf_media_stagione_norm:"Gol medi (attesi)", as_media_stagione_norm:"Assist medi (attesi)", a_media_stagione_norm:"Amm. medie (attese)", e_media_stagione_norm:"Esp. medie (attese)", gs_media_stagione_norm:"GS medi (attesi)", rp_media_stagione_norm:"RP medi (attesi)" };

const HIDE_STORICO = new Set(['aff_2024','fmand_2024','fmt_2024','mvand_2024','mvt_2024','p_2024','ruolo_2024','oldsq','a_per_presenza','as_per_presenza','e_per_presenza','gf_per_presenza','nome','oldr','sq','squadra_2024_status']);

const ICONS = { gf:'⚽', gfr:'⚪⚽', gs:'\uD83E\uDDE4', gsr:'⚪\uD83E\uDDE4', as:'\uD83C\uDFAF', a:'\uD83D\uDFE8', e:'\uD83D\uDFE5', rp:'\uD83E\uDDF1' };
const SECT_TIPS = { dati:'Medie e conteggi consolidati sulle ultime stagioni', perPres:'Indicatori normalizzati rispetto alle presenze', futures:'Valori attesi (normalizzati) stimati su base storica' };
...
function storicoSectionOrder(){ const DATI_STAT=['p_tot_4stag','cambio_ruolo_descr','p_media_stagione','aff_media_stagione','mvt_media_stagione','fmt_media_stagione','gf_media_stagione','as_media_stagione','gs_media_stagione','rp_media_stagione','a_media_stagione','e_media_stagione']; const PER_PRES=['presenze_per_gol','presenze_per_assist','presenze_per_ammonizione','presenze_per_espulsione','gs_per_presenza','rp_per_presenza','presenze_per_rp']; const FUTURES=['gf_media_stagione_norm','as_media_stagione_norm','a_media_stagione_norm','e_media_stagione_norm','gs_media_stagione_norm','rp_media_stagione_norm']; return {DATI_STAT,PER_PRES,FUTURES} }

function buildOptionsUI(){ const t=getThresholds(); const mv24=$('#opt_mv2024'), fm24=$('#opt_fm2024'), mvm=$('#opt_mvMed'), fmm=$('#opt_fmMed'); if(mv24) mv24.value=t.mv2024; if(fm24) fm24.value=t.fm2024; if(mvm) mvm.value=t.mvMed; if(fmm) fmm.value=t.fmMed; const sync=()=>saveThresholds({ mv2024:parseFloat(mv24.value||t.mv2024), fm2024:parseFloat(fm24.value||t.fm2024), mvMed:parseFloat(mvm.value||t.mvMed), fmMed:parseFloat(fmm.value||t.fmMed) }); [mv24,fm24,mvm,fmm].forEach(i=>i&&i.addEventListener('change',sync)); const g=getGroups(); const sg1=$('#switchPerPres'); const sg2=$('#switchFutures'); if(sg1) sg1.checked=g.perPres; if(sg2) sg2.checked=g.futures; sg1?.addEventListener('change',()=>{ saveGroups({ perPres: sg1.checked, futures: sg2.checked }) }); sg2?.addEventListener('change',()=>{ saveGroups({ perPres: sg1.checked, futures: sg2.checked }) }); const close=()=>{ const det=document.querySelector('details.options'); if(det) det.removeAttribute('open') }; $('#btnCloseOptions')?.addEventListener('click', close); $('#btnCloseOptionsTop')?.addEventListener('click', close); $('#btnResetOptions')?.addEventListener('click',()=>{ localStorage.removeItem(LS_THRESH); localStorage.removeItem(LS_STO_GROUPS); localStorage.removeItem(TOGGLE_KEYS.y2025); localStorage.removeItem(TOGGLE_KEYS.storico); localStorage.removeItem(THEME_KEY); setupTheme(); applyToggles(); buildOptionsUI(); alert('Impostazioni ripristinate.'); }); }

function addTopBadges(rec25,p24,srec){ const wrap=$('#playerBadges'); if(!wrap) return; const out=[]; const thr=getThresholds(); if(rec25?.cambio_ruolo===true) out.push('<span class="badge warn">Cambio Ruolo</span>'); const isCambioSq=rec25?.cambio_squadra===true; const isNew=(srec?.oldsq&&String(srec.oldsq).toLowerCase()==='new'); if(isNew) out.push('<span class="badge info">Nuovo acquisto</span>'); else if(isCambioSq) out.push('<span class="badge alert">Cambio Squadra</span>'); const mv2024=+p24?.mvt, fm2024=+p24?.fmt; if(Number.isFinite(mv2024)&&mv2024>thr.mv2024) out.push(`<span class="badge good">MV 2024 > ${thr.mv2024}</span>`); if(Number.isFinite(fm2024)&&fm2024>thr.fm2024) out.push(`<span class="badge good">FM 2024 > ${thr.fm2024}</span>`); const mvMed=+srec?.mvt_media_stagione, fmMed=+srec?.fmt_media_stagione; if(Number.isFinite(mvMed)&&mvMed>thr.mvMed) out.push(`<span class="badge good">MV medio > ${thr.mvMed}</span>`); if(Number.isFinite(fmMed)&&fmMed>thr.fmMed) out.push(`<span class="badge good">FM media > ${thr.fmMed}</span>`); let aff24=Number(p24?.aff); if(Number.isFinite(aff24)){ if(aff24>1) aff24/=100; if(aff24>0.66) out.push('<span class="badge good">Aff 2024 > 66%</span>') } const affMed=Number(srec?.aff_media_stagione); if(Number.isFinite(affMed) && affMed>0.66) out.push('<span class="badge good">Aff media > 0.66</span>'); wrap.innerHTML=out.join(' ') }

// === Shortcut remapped: press '/' to focus search (not in inputs)
document.addEventListener('keydown', (e)=>{
  ...
  if(e.key === '/' && !e.ctrlKey && !e.metaKey && !e.altKey){
    if(!isTextTarget(e.target)){
      ...
      if(el){ el.focus(); el.select?.(); showResults(); }
    }
  }
});
